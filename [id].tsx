...
import {
  View, Text, StyleSheet, TouchableOpacity, ScrollView, Alert, Linking, TextInput,
} from 'react-native';
import {
...
export default function OrderDetailScreen() {
  ...
  const openDispute = async () => {
    if (!order || !profile) return;
    Alert.prompt('سبب الاعتراض', 'اكتب سبب الاعتراض على الفاتورة', async (reason) => {
      if (!reason) return;
      ...
      if (error) {
        ...
        await supabase.from('invoices').update({ locked: true }).eq('id', invoice?.id);
      ...
      <ScrollView contentContainerStyle={styles.body}>
        ...
        {isCustomer && order.status === 'invoice_issued' && invoice && !invoice.locked && (
          <View style={styles.statusActions}>
            ...
            <TouchableOpacity style={[styles.disputeBtn, { backgroundColor: colors.error + '15', borderColor: colors.error }]} onPress={openDispute}>
