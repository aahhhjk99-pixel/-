...
export default function AdminDashboardScreen() {
  ...
  const loadData = useCallback(async () => {
    ...
    const { count: techs } = await supabase
      .from('profiles').select('*', { count: 'planned', head: true }).eq('role', 'technician');
    ...
    const { count: banned } = await supabase
      .from('profiles').select('*', { count: 'planned', head: true }).eq('account_status', 'banned');
    ...
    const { count: completedOrders } = await supabase
      .from('orders').select('*', { count: 'planned', head: true }).eq('status', 'completed');
    const { count: cancelledOrders } = await supabase
      .from('orders').select('*', { count: 'planned', head: true }).eq('status', 'cancelled');
