...
import {
  View, Text, StyleSheet, TouchableOpacity, ScrollView, RefreshControl, TextInput, Modal, ActivityIndicator,
} from 'react-native';
...
export default function CustomerHome() {
  ...
  const createOrderFromDiagnosis = async () => {
    ...
    try {
      ...
      let serviceId = (service as any)?.id;
      if (!serviceId) {
              ...
              {topTechnicians.map((tech) => {
                ...
                return (
                  <View key={tech.id} style={[styles.techCard, { backgroundColor: colors.cardBg, borderColor: colors.border }]}>
                    ...
                    <Text style={[styles.techSpecialty, { color: colors.subtext }]}>{tech.specialty || 'صيانة عامة'}</Text>
      ...
      <Modal visible={diagnosisModal} animationType="slide" transparent>
        <View style={styles.modalOverlay}>
          <View style={[styles.modalContent, { backgroundColor: colors.cardBg }]}>
            ...
            <TextInput
              style={[styles.diagnosisInput, { backgroundColor: colors.inputBg, color: colors.text, borderColor: colors.border }]}
              value={diagnosisText}
...
const styles = StyleSheet.create({
  ...
  aiDiagnosisBtn: { flexDirection: 'row', alignItems: 'center', padding: 16, borderRadius: 16, marginBottom: 24 },
  aiDiagnosisTitle: { color: '#fff', fontFamily: 'Cairo-Bold', fontSize: 15 },
  aiDiagnosisDesc: { color: 'rgba(255,255,255,0.8)', fontFamily: 'Cairo-Regular', fontSize: 12, marginTop: 2 },
  ...
  modalLabel: { fontFamily: 'Cairo-Medium', fontSize: 14, marginBottom: 8 },
  diagnosisInput: { borderWidth: 1, borderRadius: 12, padding: 12, fontFamily: 'Cairo-Regular', fontSize: 14, height: 100, marginBottom: 16 },
  diagnoseBtn: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, borderRadius: 12, paddingVertical: 14, marginBottom: 16 },
  diagnoseBtnText: { color: '#fff', fontFamily: 'Cairo-Bold', fontSize: 15 },
  ...
  resultSpecialty: { fontFamily: 'Cairo-Bold', fontSize: 16, flex: 1 },
  resultConfidence: { fontFamily: 'Cairo-Regular', fontSize: 12 },
  resultSummary: { fontFamily: 'Cairo-Regular', fontSize: 14, lineHeight: 20, marginBottom: 12 },
  createOrderBtn: { borderRadius: 10, paddingVertical: 12, alignItems: 'center' },
  createOrderBtnText: { color: '#fff', fontFamily: 'Cairo-Bold', fontSize: 14 },
});
